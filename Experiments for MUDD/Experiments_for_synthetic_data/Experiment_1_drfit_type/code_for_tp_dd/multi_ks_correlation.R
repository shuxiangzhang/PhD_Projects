...
stream_generator<-function(sd,mu,L,step_size,d){
  rho<-0.2
  mu<-rep(mu,d)
...
detect<-function(sd,mu,L,step_size,d){
  TP<-0
  DD<-NULL
  D_time<-NULL
  MEM<-NULL
  ...
  for (i in seq(30)){
    dd<-NULL
    signal<-FALSE
    stream<-stream_generator(sd,mu,L,step_size,d)
    win1<-stream[1:100,]
    win2<-stream[101:200,]
    detect<-NULL
    flag<-TRUE
    ...
    for (j in 201:L){
      ...
      if (flag==TRUE){
        if (j-latest_point == 1){
          ...
          win<-rbind(win1,win2)
          start_time = Sys.time()
          result<-KStest(object=t(win), group=c(rep(1,100),rep(2,100))) 
          end_time = Sys.time()
  ...
  write.csv(df,file = paste('final_result_ks_rho_abrupt','_',d,'_',L,'_','.csv',sep = ''))
}


set.seed(0)
detect(sd = 0.2,mu = 0.2,L = 50000,step_size = 0.5,d = 2)
