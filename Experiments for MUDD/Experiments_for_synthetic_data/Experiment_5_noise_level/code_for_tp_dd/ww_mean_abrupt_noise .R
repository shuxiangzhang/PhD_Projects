...
detect<-function(rho,sd,L,step_size,d,level){
  ...
  for (i in seq(30)){
    ...
    stream<-noise_masking(stream,level,0.5,sd,d)
    win1<-stream[1:100,]
    win2<-stream[101:200,]
    detect<-NULL
    flag<-TRUE
    ...
    for (j in 201:L){
      ...
      if (flag==TRUE){
        if (j-latest_point == 1){
          ...
          win<-rbind(win1,win2)
          start_time = Sys.time()
          result<-WWtest(object=t(win), group=c(rep(1,100),rep(2,100)),nperm=100) 
          end_time = Sys.time()
  ...
  write.csv(df,file = paste('final_result_ww_mean_abrupt','_',d,'_',L,'_',level,'.csv',sep = ''))
}
