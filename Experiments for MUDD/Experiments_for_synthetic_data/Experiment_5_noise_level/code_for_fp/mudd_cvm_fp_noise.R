...
detect<-function(rho,sd,L,level,d){
  fp<-NULL
  MEM<-NULL
  T_time<-NULL
  for (i in seq(30)){
  ...
  write.csv(df,file = paste('uni_cvm_fp_',level,'_',L,'.csv',sep = ''))
}

set.seed(0)
for (l in seq(0.05,0.3,0.05)){
  detect(rho = 0.2,sd = 0.2,L = 50000, level = l,d=2)
}
