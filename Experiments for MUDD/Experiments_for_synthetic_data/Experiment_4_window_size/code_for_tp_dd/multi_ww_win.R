...
detect<-function(rho,sd,L,step_size,d,win_size){
  TP<-0
  DD<-NULL
  D_time<-NULL
  MEM<-NULL
    ...
    for (j in 201:L){
      ...
      if (flag==TRUE){
        if (j-latest_point == 1){
          ...
          win<-rbind(win1,win2)
          start_time = Sys.time()
          result<-WWtest(object=t(win), group=c(rep(1,win_size),rep(2,win_size)),nperm=1000) 
          end_time = Sys.time()
  ...
  write.csv(df,file = paste('final_result_ww_mean_abrupt','_',d,'_',L,'_',win_size,'.csv',sep = ''))
}


set.seed(0)
for (win_size in seq(50,200,10)){
  detect(rho = 0.2,sd = 0.2,L = 50000,step_size = 0.5,d = 2,win_size = win_size)
}
