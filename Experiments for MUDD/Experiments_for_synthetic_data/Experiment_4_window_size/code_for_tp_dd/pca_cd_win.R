...
detect<-function(rho,sd,L,step_size,d,win_size){
  ...
  for (w in seq(30)){
    ...
    for (n in (2*win_size+1):L){
      ...
      threshold<-0.05*win_size*kl_mean
      if (n%%win_size==0){
        m<-win_size
      }else{
        m<-n%%win_size
  ...
  write.csv(df,file = paste('PCA-CD_mean_abrupt_win_size','_',d,'_',L,'_',win_size,'.csv',sep = ''))
}
