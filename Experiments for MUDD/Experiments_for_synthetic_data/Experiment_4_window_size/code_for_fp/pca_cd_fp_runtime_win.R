...
run<-function(rho,sd,L,win_size){
  ...
  for (w in seq(30)){
    ...
    for (u in (2*win_size+1):nrow(stream)){
      kl_x_y<-rep(0,k)
      kl_y_x<-rep(0,k)
      kl<-rep(0,k)
      start_time = Sys.time()
  ...
  write.csv(df,file = paste('pca_cd_win_fp_',win_size,'.csv',sep = ''))
}

set.seed(0)
for (win_size in seq(50,200,10)){
  run(rho = 0.2,sd = 0.2,L = 50000, win_size = win_size)
}