...
detect<-function(rho,sd,L,step_size,d){
    ...
    for (j in 201:L){
      ...
      if (flag==TRUE){
        if (j-latest_point == 1){
          ...
          win<-rbind(win1,win2)
          start_time = Sys.time()
          result<-WWtest(object=t(win), group=c(rep(1,100),rep(2,100)),nperm=1000) 
          end_time = Sys.time()
  ...
  write.csv(df,file = paste('final_result_ww_mean_abrupt','_',d,'_',L,'_','.csv',sep = ''))
}


set.seed(0)
for (d in seq(50,1000,50)){
  detect(rho = 0.2,sd = 0.2,L = 50000,step_size = 0.5,d = d)
}
