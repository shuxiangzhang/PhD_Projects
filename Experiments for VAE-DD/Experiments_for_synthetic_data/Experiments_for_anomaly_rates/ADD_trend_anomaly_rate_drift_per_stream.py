...
DRIFT = {}
for anomaly_rate in [0,0.02,0.04,0.06,0.08,0.1]:
...
#                             stream.iloc[i,data_dim] = stream.iloc[i,data_dim]+np.random.normal(loc=0, scale=noise_degree*drift_mag,size=1)[0]   
    ...
    file_name_1 = 'add_trend_anomaly_rate_'+str(anomaly_rate)
    a_file = open(file_name_1, "w")
    json.dump(Drift, a_file)
    a_file.close()
    
file_name_1 = 'add_trend_anomaly_rate_total_'+str(anomaly_rate)
a_file = open(file_name_1, "w")
json.dump(DRIFT, a_file)
a_file.close()
