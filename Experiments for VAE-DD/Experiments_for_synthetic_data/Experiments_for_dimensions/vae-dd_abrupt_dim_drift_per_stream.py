...
#                 stream.iloc[i,] = data_generator(mu_c,mu_d,sigma_a,sigma_b,rho_a,rho_b,dim,1)[0]
        ...
        for i in range(stream.shape[0]):
            ...
            memory.append(asizeof.asizeof(vae_dd))
        end_time = time.time()
    ...
    file_name_2 = 'vae_dd_dim_mem_'+str(dim)
    b_file = open(file_name_2, "w")
    json.dump(Mem, b_file)
    b_file.close()
    
    file_name_3 = 'vae_dd_dim_time_'+str(dim)
...
file_name_2 = 'vae_dd_dim_mem_total_'+str(dim)
b_file = open(file_name_2, "w")
json.dump(MEM, b_file)
b_file.close()

file_name_3 = 'vae_dd_dim_time_total_'+str(dim)
c_file = open(file_name_3, "w")
json.dump(TIME, c_file)
c_file.close()
