...
def IBDD(train_X, test_X, window_length, consecutive_values):
    ...
    files2del = ['wj.jpeg', 'wk.jpeg', 'wj_cv.jpeg', 'wk_cv.jpeg']
    print(test_X.shape)
    n_runs = 20
    ...
    drift_points = []
    w1 = get_imgdistribution("wj.jpeg", recent_data_X)
    lastupdate = 0
    start = timer()
    print('IBDD Running...')
    memory = []
    for i in range(test_X.shape[0]): 
        ...
        w2 = get_imgdistribution("wk.jpeg", recent_data_X)
        ...
        if (all(i >= superior_threshold for i in nrmse[-consecutive_values:])):
            ...
            print(f'drift was detected at {i}')
            lastupdate = i
        memory.append(asizeof.asizeof(recent_data_X)+asizeof.asizeof(nrmse)+asizeof.asizeof(w2)+asizeof.asizeof(threshold_diffs)+asizeof.asizeof(drift_points))
    return drift_points, np.mean(memory)
...
def find_initial_threshold(X_train, window_length, n_runs):
    ...
    w1_cv = get_imgdistribution("wj_cv.jpeg", w1)
    ...
    for i in range(0,n_runs):
        ...
        w2_cv = get_imgdistribution("wk_cv.jpeg", w2)
...
#                 stream.iloc[i,] = data_generator(mu_c,mu_d,sigma_a,sigma_b,rho_a,rho_b,dim,1)[0]
        ...
        drifts,mem = IBDD(stream_train,stream,window_size, epsilon)
        end_time = time.time()
    ...
    file_name_1 = 'ibdd_dim_'+str(dim)
    a_file = open(file_name_1, "w")
    json.dump(Drift, a_file)
    a_file.close()
    
    
    file_name_2 = 'ibdd_dim_mem_'+str(dim)
    b_file = open(file_name_2, "w")
    json.dump(Mem, b_file)
    b_file.close()
    
    file_name_3 = 'ibdd_dim_time_'+str(dim)
    c_file = open(file_name_3, "w")
    json.dump(Time, c_file)
    c_file.close()
    
file_name_1 = 'ibdd_dim_total_'+str(dim)
a_file = open(file_name_1, "w")
json.dump(DRIFT, a_file)
a_file.close()


file_name_2 = 'ibdd_dim_mem_total_'+str(dim)
b_file = open(file_name_2, "w")
json.dump(MEM, b_file)
b_file.close()

file_name_3 = 'ibdd_dim_time_total_'+str(dim)
c_file = open(file_name_3, "w")
json.dump(TIME, c_file)
c_file.close()
