...
def IBDD(train_X, test_X, window_length, consecutive_values):
    ...
    files2del = ['we.jpeg', 'wf.jpeg', 'we_cv.jpeg', 'wf_cv.jpeg']
    print(test_X.shape)
    n_runs = 20
    ...
    drift_points = []
    w1 = get_imgdistribution("we.jpeg", recent_data_X)
    lastupdate = 0
    start = timer()
    print('IBDD Running...')
    for i in range(test_X.shape[0]): 
        ...
        w2 = get_imgdistribution("wf.jpeg", recent_data_X)
...
def find_initial_threshold(X_train, window_length, n_runs):
    ...
    w1_cv = get_imgdistribution("we_cv.jpeg", w1)
    ...
    for i in range(0,n_runs):
        ...
        w2_cv = get_imgdistribution("wf_cv.jpeg", w2)
...
dim = 10
DRIFT = {}
for noise_degree in [1,1.2,1.4,1.6,1.8,2.0]:
    print(f'noise_degree:{noise_degree}')
    Drift = []
...
#                 stream.iloc[i,] = data_generator(mu_c,mu_d,sigma_a,sigma_b,rho_a,rho_b,dim,1)[0]
        ...
        Drift.append(drifts)
    file_name_1 = 'ibdd_noise_degree_'+str(noise_degree)
    a_file = open(file_name_1, "w")
    json.dump(Drift, a_file)
    a_file.close()   
    DRIFT[noise_degree] = Drift
file_name_1 = 'ibdd_noise_degree_total_'+str(noise_degree)
a_file = open(file_name_1, "w")
json.dump(DRIFT, a_file)
a_file.close()   
