...
stream_generator<-function(dim,m,k,slope){
  ...
  last<-paste(',MGC_Static(dens=0.5, par=0.5, center=rep(',(1+(k-1)*7),',dim))',',MGC_Static(dens=0.5, par=0.5, center=rep(',(m+1+(k-1)*7),',dim))')
  ex<-paste('DSD_MG(dim = dim,',ex,last,')')
  stream_2<- eval(parse(text = ex))
  stream_2<-get_points(stream_2,k*500)
  stream_3<-NULL
  for(d in seq((0.5/slope))){
    s<-DSD_MG(dim = dim, MGC_Static(dens=1, par=0.5, center=rep(1,dim)) ,MGC_Static(dens=1-slope*d, par=0.5, center=rep( 8 ,dim)) ,MGC_Static(dens=slope*d, par=0.5, center=rep( 8+m ,dim)))
    d<-get_points(s,1)
...
detect<-function(dim,m,k,slope){
  ...
  ground_truth<-rep(c(rep(0,(k-1)),5),count_partition)
  final_result$ground_truth<-ground_truth
  write.csv(final_result,paste('slope_',slope,'_cluster_2_type_5.csv',sep = ''),row.names = FALSE)
}

set.seed(0)
for(slope in seq(0.01,0.05,0.01)){
  detect(2,2,2,slope)
}
