...
stream_generator<-function(dim,m,k,slope){
  ...
  last<-paste(',MGC_Static(dens=1, par=0.5-m, center=rep(',(1+(k-1)*7),',dim))')
  ex<-paste('DSD_MG(dim = dim,',ex,last,')')
  stream_2<- eval(parse(text = ex))
  stream_2<-get_points(stream_2,k*500)
  
  stream_3<-NULL
  for(d in seq(m/slope)){
    s<-DSD_MG(dim = dim, MGC_Static(dens=1, par=0.5, center=rep(1,dim)) ,MGC_Static(dens=1, par=0.5-slope*d, center=rep( 8 ,dim)))
    d<-get_points(s,1)
    stream_3<-rbind(stream_3,d)
  }
  stream<-rbind(stream_1,stream_3,stream_2)
  stream
}
...
detect<-function(dim,m,k,slope){
  final_result<-data.frame(matrix(nrow= 1,ncol = 4))
  for (b in seq(30)){
    batch_num<-b
    stream<-stream_generator(dim,m,k,slope)
    win1<-stream[1:(k*100),]
      ...
      if (flag){
        ...
        if (counter==(k*100)){
          ...
          for (par_num in seq(k)){      
            ...
            }else if (intensity[par_num]>0.2 && intensity[par_num]<0.9 && localityRatio[par_num]>0.1&& localityRatio[par_num]<0.9){
              drift_type<-2
              final_result<-rbind(final_result,c(partition_num,drift_type,drift_time,batch_num))
            ...
            }else if (intensity[par_num]>0.2 && intensity[par_num]<0.9 && localityRatio[par_num]<=0.1){
              ave_1<-colMeans(original_data_per_partition[[real_num]])
              ave_2<-colMeans(remaining_data_per_partition[[real_num]])
              f_1<-apply(original_data_per_partition[[real_num]],1,function(x) dist(rbind(x,ave_1)))
              f_2<-apply(remaining_data_per_partition[[real_num]],1,function(x) dist(rbind(x,ave_1)))
  ...
  ground_truth<-rep(c(rep(0,(k-1)),1),count_partition)
  final_result$ground_truth<-ground_truth
  write.csv(final_result,paste('slope_',slope,'_cluster_2_type_1.csv',sep = ''),row.names = FALSE)
}

set.seed(0)
for(slope in seq(0.001,0.005,0.001)){
  detect(2,0.4,2,slope)
}
