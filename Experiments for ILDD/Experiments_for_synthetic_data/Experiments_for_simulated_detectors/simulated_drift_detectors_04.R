...
stream_generator<-function(dim,m){
  ...
  stream_2<- DSD_MG(dim = dim,
                    MGC_Static(dens=1, par=0.5, center=rep(1,dim)),
                    MGC_Static(dens=1, par=(0.5+m), center=rep(8,dim))
  )
...
detect<-function(dim,k,m,acc_g,acc_l){
  ...
  ground_truth<-rep(c(0,4),count_partition)
  final_result$ground_truth<-ground_truth
  write.csv(final_result,paste('acc_',acc_g,'_',acc_l,'_final_result_04_','.csv'),row.names = FALSE)
}
