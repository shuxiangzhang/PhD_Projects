install.packages('stream')
install.packages('ggpubr')
install.packages('factoextra')
install.packages('philentropy')
install.packages('MASS')
install.packages('dplyr')
install.packages('cramer')
install.packages('LICORS')
if (!requireNamespace("BiocManager", quietly = TRUE))
  install.packages("BiocManager")
BiocManager::install("GSAR")
install.packages('pryr')
library(LICORS)
library(philentropy)
library(stream)
library(ggpubr)
library(factoextra)
library(pryr)
library(GSAR)
library(dplyr)
library(cramer)
library(MASS)


stream_generator<-function(dim,m){
  stream_1<- DSD_MG(dim = dim,
                    MGC_Static(dens=1, par=0.5, center=rep(1,dim)),
                    MGC_Static(dens=1, par=0.5, center=rep(8,dim)))
  
  stream_1<-get_points(stream_1,1000)
  
  stream_2<- DSD_MG(dim = dim,
                    MGC_Static(dens=1, par=0.5, center=rep(1,dim)),
                    MGC_Static(dens=0.5, par=0.5, center=rep(8,dim)),
                               MGC_Static(dens=0.5, par=0.5, center=rep(8+m,dim)))
  
  
  stream_2<-get_points(stream_2,1000)
  stream<-rbind(stream_1,stream_2)
}


detect<-function(dim,k,m,acc_g,acc_l){
  final_result<-data.frame(matrix(nrow= 1,ncol = 4))
  for (i in seq(30)){
    batch_num<-i
    stream<-stream_generator(dim,m)
    win1<-stream[1:(k*100),]
    win2<-stream[((k*100)+1):2*(k*100),]
    detect<-NULL
    flag<-TRUE
    counter<-0
    for (j in (2*(k*100)+1):nrow(stream)){
      win2<-win2[-1,]
      win2<-rbind(win2,stream[j,])
      
      if (flag){
        if (j==1000){
          prob<-runif(1,0,1)
          if (prob>=(1-acc_g)){
            p_value<-0.01
          }else{
            p_value<-1
          }
        }else{
          prob<-runif(1,0,1)
          p_value<-ifelse(prob>=0.01,1,0.01)
          
        }
        if (p_value<=0.05){
          drift_time<-j
          print(paste('Drift was detected at:',j))
          flag<-FALSE
        }
      }else{
        counter<-counter+1
        if (counter==(k*100)){
          res.km<-kmeanspp(win1,k)
          assignment<-res.km$cluster
          names(assignment)<-NULL
          original_par_size<-res.km$size
          original_data_per_partition<-list()
          radius_per_partition<-list()
          remaining_par_size<-list()
          remaining_data_per_partition<-list()
          
          for (i in seq(k)){
            x<-win1[which(assignment==i),]
            original_data_per_partition[[i]]<-x
            y<-res.km$centers[i,]
            radius<-max(apply(x,1,euc.dist<-function(x1, x2=y) sqrt(sum((x1 - x2) ^ 2))))
            radius_per_partition[[i]]<-radius
            remaining_par_size[[i]]<-0
            remaining_data_per_partition[[i]]<-data.frame(matrix(nrow=1,ncol = dim))
          }
          
          for (j in 1:nrow(win2)){
            min_dist<-min(apply(res.km$centers,1,euc.dist<-function(x1, x2=win2[j,]) sqrt(sum((x1 - x2) ^ 2))))
            partition_index<-which.min(apply(res.km$centers,1,euc.dist<-function(x1, x2=win2[j,]) sqrt(sum((x1 - x2) ^ 2))))
            if(min_dist<=radius_per_partition[[partition_index]]){
              remaining_par_size[[partition_index]]<-remaining_par_size[[partition_index]]+1
              remaining_data_per_partition[[partition_index]]<-rbind(remaining_data_per_partition[[partition_index]],win2[j,])
            }
          }
          remaining_par_size<-unlist(remaining_par_size)
          intensity<-abs((original_par_size-remaining_par_size))/original_par_size
          
          localityRatio<-NULL
          for (i in seq(k)){
            centroid_remaining<-colMeans(remaining_data_per_partition[[i]],na.rm = TRUE)
            if (is.nan(centroid_remaining[1])){
              localityRatio<-c(localityRatio,1)
            }else{
              localityRatio<-c(localityRatio,euc.dist(centroid_remaining,res.km$centers[i,])/radius_per_partition[[i]])
            }
          }
          print(intensity)
          print(localityRatio)
          print(res.km$centers)
          print(original_par_size)
          
          # Make the partition number consisitent
          first_coordinate_of_center<-unname(res.km$centers[,1])
          corresponding_partion_for_centroid<-order(first_coordinate_of_center)
          locality<-res.km$centers[corresponding_partion_for_centroid,]
          localityRatio<-localityRatio[corresponding_partion_for_centroid]
          intensity<-intensity[corresponding_partion_for_centroid]
          
          # Determine drift type based on the intensity and localityRatio
          for (i in seq(k)){      
            real_num<-corresponding_partion_for_centroid[i]
            if (intensity[i]<=0.2){
              if (i==2){
                print(paste('intensity for 2:',intensity[i]))
                prob<-runif(1,0,1)
                prob
                if (prob<=acc_l){
                  p_value<-0.01
                }else{
                  p_value<-1
                }
              }else{
                
                prob<-runif(1,0,1)
                if (prob<=(1-acc_l)){
                  p_value<-0.01
                }else{
                  p_value<-1
                }
              }
              
              if (p_value<0.05){
                drift_type<-1
                final_result<-rbind(final_result,c(i,drift_type,drift_time,batch_num))
                print(paste('shrinks or skewness drift for partition:', locality[i,1],locality[i,2]))
              }else{
                drift_type<-0
                final_result<-rbind(final_result,c(i,drift_type,drift_time,batch_num))
                print(paste('no drift for partition:', locality[i,1],locality[i,2]))
              }
              
            }else if (intensity[i]>0.2 && intensity[i]<0.9 && localityRatio[i]>0.24&& localityRatio[i]<0.9){
              drift_type<-2
              final_result<-rbind(final_result,c(i,drift_type,drift_time,batch_num))
              print(paste('partially moving for partition:', locality[i,1],locality[i,2]))
            }else if (intensity[i]>=0.9 || localityRatio[i]>=0.9){
              drift_type<-3
              final_result<-rbind(final_result,c(i,drift_type,drift_time,batch_num))
              print(paste('completely moving for partition:', locality[i,1],locality[i,2]))
            }else if (intensity[i]>0.2 && localityRatio[i]<=0.24){
              
              if (i==2){
                prob<-runif(1,0,1)
                if (prob>=acc_l){
                  p_value<-0.01
                }else{
                  p_value<-1
                }
              }else{
                prob<-runif(1,0,1)
                p_value<-ifelse(prob>=0.01,1,0.01)
              }
              if (p_value<0.05){
                drift_type<-4
                final_result<-rbind(final_result,c(i,drift_type,drift_time,batch_num))
                print(paste('Expanding for partition:', locality[i,1],locality[i,2]))
              }else{
                drift_type<-5
                final_result<-rbind(final_result,c(i,drift_type,drift_time,batch_num))
                print(paste('Falsely staying for partition:', locality[i,1],locality[i,2]))
              }
            }else{
              drift_type<-6
              final_result<-rbind(final_result,c(i,drift_type,drift_time,batch_num))
              print(paste('No such type', locality[i,1],locality[i,2]))
            }
          }
          win1<-win2
        }
        if (counter==2*(k*100)){
          flag<-TRUE
          counter<-0
        }
      }
      
    }
    print(paste('This is run:',batch_num))
  }
  colnames(final_result)<-c('partition_num','drift_type','drift_time','batch_num')
  final_result<-final_result[-1,]
  final_result<-final_result[order(final_result$partition_num),]
  print(final_result)
  count_partition<-as.numeric(table(final_result$partition_num))
  print(count_partition)
  ground_truth<-rep(c(0,5),count_partition)
  final_result$ground_truth<-ground_truth
  write.csv(final_result,paste('acc_',acc_g,'_',acc_l,'_final_result_05_','.csv',sep=''),row.names = FALSE)
}

set.seed(0)

for(acc_g in seq(0.7,1,0.1)){
  for(acc_l in seq(0.7,1,0.05)){
  detect(2,2,2,acc_g,acc_l)}
}





