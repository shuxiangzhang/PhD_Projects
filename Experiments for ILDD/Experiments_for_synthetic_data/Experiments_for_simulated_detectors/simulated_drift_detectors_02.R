...
stream_generator<-function(dim,m){
  ...
  stream_2<- DSD_MG(dim = dim,
                    MGC_Static(dens=1, par=0.5, center=rep(1,dim)),
                    MGC_Static(dens=1, par=(0.5), center=rep(8+m,dim))
  )
...
detect<-function(dim,k,m,acc_g,acc_l){
      ...
      if (flag){
        ...
        if (counter==(k*100)){
          ...
          for (i in seq(k)){      
            ...
            }else if (intensity[i]>0.2 && intensity[i]<0.9 && localityRatio[i]>0.3&& localityRatio[i]<0.9){
              drift_type<-2
              final_result<-rbind(final_result,c(i,drift_type,drift_time,batch_num))
            ...
            }else if (intensity[i]>0.2 && localityRatio[i]<=0.3){
  ...
  ground_truth<-rep(c(0,2),count_partition)
  final_result$ground_truth<-ground_truth
  write.csv(final_result,paste('acc_',acc_g,'_',acc_l,'_final_result_02_','.csv',sep=''),row.names = FALSE)
}
set.seed(0)
for(acc_g in seq(0.7,1,0.1)){
  for(acc_l in seq(0.7,1,0.05)){
    detect(2,2,1,acc_g,acc_l)
  }
