...
detect<-function(dim,k,m,acc_g,acc_l){
  ...
  ground_truth<-rep(c(0,1),count_partition)
  final_result$ground_truth<-ground_truth
  write.csv(final_result,paste('acc_',acc_g,'_',acc_l,'_final_result_01_','.csv',sep=''),row.names = FALSE)
}

set.seed(0)
acc_g<-0.7
for(acc_l in seq(0.7,1,0.05)){
  detect(2,2,0.4,acc_g,acc_l)
}
