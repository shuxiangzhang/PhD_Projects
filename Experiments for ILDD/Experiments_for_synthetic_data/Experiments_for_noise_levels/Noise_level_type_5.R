...
noise_masking<-function(stream,level,dim){
  low<-1-3*0.5
  up<-8+3*0.5
  data_point<-runif(dim,low,up)
...
detect<-function(dim,m,k,level){
  ...
  write.csv(final_result,paste('noise_',level,'_cluster_2_type_5.csv',sep = ''),row.names = FALSE)
}

set.seed(0)
for(level in seq(0.05,0.3,0.05)){
  detect(2,2,2,level)
}
