...
stream_name = "sine1"
project = Project("projects/single", "sled_sine1_abrupt_training/" + stream_name)

# 2. Generating training stream

project_path = "data_streams/sled_sine1_abrupt_training/" + stream_name + "/"
if not os.path.exists(project_path):
    os.makedirs(project_path)
train_size = 200
file_path = project_path + stream_name
stream_generator =SINE1(concept_length=5000,noise_rate=0.1,transition_length=5,num_drifts=100)
...
for final_w in fw:
    ...
    # Creating a project
    stream_name = "sine1"
    project = Project("projects/single", "sled_sine1_abrupt_testing/" + stream_name)
    # Generating training stream

    project_path = "data_streams/sled_sine1_abrupt_testing/" + stream_name + "/"
    if not os.path.exists(project_path):
        os.makedirs(project_path)
    ...
    f = open('sled_sine1_abrupt.txt', 'a')
