...
stream_name = "circle"
project = Project("projects/single", "sled_circle_abrupt_training/" + stream_name)

# 2. Generating training stream

project_path = "data_streams/sled_circle_abrupt_training/" + stream_name + "/"
...
stream_generator = CIRCLES(concept_length=5000, noise_rate=0.1, transition_length=5, thresholds=thresholds)
...
for final_w in fw:
    ...
    # Creating a project
    stream_name = "circle"
    project = Project("projects/single", "sled_circle_abrupt_testing/" + stream_name)
    # Generating training stream

    project_path = "data_streams/sled_circle_abrupt_testing/" + stream_name + "/"
    if not os.path.exists(project_path):
        os.makedirs(project_path)
    ...
    f = open('sled_circle_abrupt.txt', 'a')
