...
stream_name = "led"
project = Project("projects/single", "sled_led_moderate_training/" + stream_name)

# 2. Generating training stream

project_path = "data_streams/sled_led_moderate_training/" + stream_name + "/"
...
stream_generator = LEDConceptDrift(concept_length=5000, noise_rate=0.1, transition_length=500, led_attr_drift=led_attr_drift)
...
for final_w in fw:
    ...
    # Creating a project
    stream_name = "led"
    project = Project("projects/single", "sled_led_moderate_testing/" + stream_name)
    # Generating training stream

    project_path = "data_streams/sled_led_moderate_testing/" + stream_name + "/"
    if not os.path.exists(project_path):
        os.makedirs(project_path)

    for tm in range(repeat_count):
        stream_generator = LEDConceptDrift(concept_length=5000, noise_rate=0.1, transition_length=500,
    ...
    f = open('sled_led_moderate.txt', 'a')
