...
stream_name = "circle"
project = Project("projects/single", "sled_circle_moderate_training/" + stream_name)

# 2. Generating training stream

project_path = "data_streams/sled_circle_moderate_training/" + stream_name + "/"
if not os.path.exists(project_path):
    os.makedirs(project_path)
train_size = 200
file_path = project_path + stream_name
thresholds = [[[0.2, 0.5], 0.15], [[0.4, 0.5], 0.2], [[0.6, 0.5], 0.25], [[0.8, 0.5], 0.3]] * 25
thresholds.append([[0.2, 0.5], 0.15])

stream_generator = CIRCLES(concept_length=5000, noise_rate=0.1, transition_length=500, thresholds=thresholds)
...
for final_w in fw:
    ...
    # Creating a project
    stream_name = "circle"
    project = Project("projects/single", "sled_circle_moderate_testing/" + stream_name)
    # Generating training stream

    project_path = "data_streams/sled_circle_moderate_testing/" + stream_name + "/"
    if not os.path.exists(project_path):
        os.makedirs(project_path)
    ...
    f = open('sled_circle_moderate.txt', 'a')
