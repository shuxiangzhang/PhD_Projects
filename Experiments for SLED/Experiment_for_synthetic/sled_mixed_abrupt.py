...
stream_name = "mixed"
project = Project("projects/single", "sled_mixed_abrupt_training/" + stream_name)

# 2. Generating training stream

project_path = "data_streams/sled_mixed_abrupt_training/" + stream_name + "/"
if not os.path.exists(project_path):
    os.makedirs(project_path)
train_size = 200
file_path = project_path + stream_name
stream_generator = MIXED(concept_length=5000, noise_rate=0.1,transition_length=5, num_drifts=100)
...
for final_w in fw:
    ...
    # Creating a project
    stream_name = "mixed"
    project = Project("projects/single", "sled_mixed_abrupt_testing/" + stream_name)
    # Generating training stream

    project_path = "data_streams/sled_mixed_abrupt_testing/" + stream_name + "/"
    if not os.path.exists(project_path):
        os.makedirs(project_path)
    ...
    f = open('sled_mixed_abrupt.txt', 'a')
